...
#include "EGGraphStormWidgetsNACL.h"
...
static const char class_name[] = "EGGraphStormWidgetsNACL";
static const char* rsrcInFilename = "/http/Resources.zip";
static const char* rsrcOutFilename = "/tmp/Resources.zip";

EGGraphStormInstanceMap EGGraphStormWidgetsNACL::instances;

EGGraphStormWidgetsNACL::EGGraphStormWidgetsNACL(PP_Instance instance)
    : widgets(new EGGraphStormWidgets()), instance(instance), graphicsInited(false), rsrcLoaded(false), appInited(false), inSwapBuffers(false)
{
}

void EGGraphStormWidgetsNACL::graphicsInit(int width, int height)
{
...
void EGGraphStormWidgetsNACL::appInit()
{
...
void EGGraphStormWidgetsNACL::message(std::string message)
{
    messageLock.lock();
...
void EGGraphStormWidgetsNACL::postMessage(void* userdata, int)
{
    EGGraphStormWidgetsNACL *nacl = (EGGraphStormWidgetsNACL*)userdata;
...
void EGGraphStormWidgetsNACL::processEvent(PP_Resource input_event)
{
...
void EGGraphStormWidgetsNACL::display(EGbool reschedule)
{
    if (inSwapBuffers) {
        if (reschedule) {
            PP_CompletionCallback cc = PP_MakeCompletionCallback(EGGraphStormWidgetsNACL::displaySchedule, this);
            ppb_core_interface->CallOnMainThread(1000 / EGGraphStormWidgetsNACL::HZ, cc, 0);
        }
        return;
    }
    if (!rsrcLoaded) {
    ...
    inSwapBuffers = true;
    if (reschedule) {
        PP_CompletionCallback cc = PP_MakeCompletionCallback(EGGraphStormWidgetsNACL::displayDoneSchedule, this);
        ppb_g3d_interface->SwapBuffers(context, cc);
    } else {
        PP_CompletionCallback cc = PP_MakeCompletionCallback(EGGraphStormWidgetsNACL::displayDone, this);
        ppb_g3d_interface->SwapBuffers(context, cc);
    }
}

void EGGraphStormWidgetsNACL::displaySchedule(void *userdata, int)
{
    EGGraphStormWidgetsNACL *nacl = (EGGraphStormWidgetsNACL*)userdata;
    nacl->display(true);
}

void EGGraphStormWidgetsNACL::displayDoneSchedule(void *userdata, int)
{
    EGGraphStormWidgetsNACL *nacl = (EGGraphStormWidgetsNACL*)userdata;
    nacl->inSwapBuffers = false;
    PP_CompletionCallback cc = PP_MakeCompletionCallback(EGGraphStormWidgetsNACL::displaySchedule, nacl);
    ppb_core_interface->CallOnMainThread(1000 / EGGraphStormWidgetsNACL::HZ, cc, 0);
}

void EGGraphStormWidgetsNACL::displayDone(void *userdata, int)
{
    EGGraphStormWidgetsNACL *nacl = (EGGraphStormWidgetsNACL*)userdata;
    nacl->inSwapBuffers = false;
}
...
static void* LoadResources(void* userdata)
{
    ...
    // drop our thread priority and choose round robin scheduling
                                  ...
                                  const char* argv[])
{
    ...
    // create instance
    EGGraphStormWidgetsNACL *nacl = new EGGraphStormWidgetsNACL(instance);
    EGGraphStormWidgetsNACL::instances.insert(EGGraphStormInstanceEntry(instance, EGGraphStormWidgetsNACLPtr(nacl)));
...
{
    EGGraphStormInstanceIterator nacli = EGGraphStormWidgetsNACL::instances.find(instance);
    if (nacli != EGGraphStormWidgetsNACL::instances.end()) {
        EGGraphStormWidgetsNACL *nacl = (*nacli).second.get();
        nacl->processEvent(input_event);
    }
...
{
    EGDebug("%s:%s\n", class_name, __func__);
    EGGraphStormWidgetsNACL::instances.erase(instance);
}
                                   ...
                                   PP_Resource view_resource)
{
    ...
    if (nacli != EGGraphStormWidgetsNACL::instances.end()) {
        ...
        if (!nacl->graphicsInited) {
            ...
            glViewport(0, 0, position.size.width, position.size.height);
        }
        nacl->widgets->reshape(position.size.width, position.size.height);
    }
}
