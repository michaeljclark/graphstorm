...
_cairo_stroker_line_to (void *closure,
			const cairo_point_t *point, const cairo_slope_t *slope) /* XXX EMSCRIPTEN */
{
    cairo_stroker_t *stroker = closure;
...
_cairo_stroker_line_to_dashed (void *closure,
			       const cairo_point_t *p2, const cairo_slope_t *slope) /* XXX EMSCRIPTEN */
{
    cairo_stroker_t *stroker = closure;
...
_cairo_stroker_close_path (void *closure)
{
	...
	status = _cairo_stroker_line_to_dashed (stroker, &stroker->first_point, NULL); /* XXX EMSCRIPTEN */
    else
	status = _cairo_stroker_line_to (stroker, &stroker->first_point, NULL); /* XXX EMSCRIPTEN */
    if (unlikely (status))
	return status;
