 ...
 */
static cairo_status_t
line_to_dashed (void *closure, const cairo_point_t *point, const cairo_slope_t *slope) /* XXX EMSCRIPTEN */
{
    struct stroker *stroker = closure;
...
close_path_dashed (void *closure)
{
    ...
    status = line_to_dashed (stroker, &stroker->first_point, NULL); /* XXX EMSCRIPTEN */
    if (unlikely (status))
	return status;
